...
Betrachten wir ein Nullsummenspiel mit zwei Akteuren names \textsc{Max} und
\textsc{Min}, deren Ziele einander widersprechen.
...
\begin{equation}
    ...
    \begin{cases}
        ...
        \text{max}_{a \in A(s)} \text{\textsc{minimax}}(\text{\textsc{succ}}(s, a)) & \text{falls \textsc{to-move}}(s) = \text{\textsc{Max}}, \\
        \text{min}_{a \in A(s)} \text{\textsc{minimax}}(\text{\textsc{succ}}(s, a)) & \text{falls \textsc{to-move}}(s) = \text{\textsc{Min}}.
    \end{cases}
    \label{eq:minimax}
\end{equation}
